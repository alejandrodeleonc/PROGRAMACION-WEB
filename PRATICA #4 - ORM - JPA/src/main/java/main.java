...
public class main {
    public static void main(String[] args) {
        ...
        /*for(int i = 0; i<10; i++){
            UsuarioServices.getInstancia().crear(new Usuario("usuario"+i, "Prueba"+i, String.valueOf(i)));
        }
        List<ventaprod> pure = new ArrayList<ventaprod>();
        for(int b = 0; b<5; b++){
            ProductoServices.getInstancia().crear(new Producto(b, "Prueba "+b, 5, new BigDecimal(580.2)));
            pure.add(new ventaprod(prueba.get(0), 5 ));
        }

        Date g = new Date();

        VentasServices.getInstancia().crear(new VentasProductos("1", g, "Alejandro", pure, 25000));*/
        ...
        Javalin app = Javalin.create(config ->{
            ...
            app.post("/login", ctx -> {
                ...
                print("encapsulacion.Usuario: "+ usuario);
            ...
            app.post("/editar", ctx -> {
                ...
                if(control.editarProducto(tmp) == null){
                    modelo.put("error","Error: encapsulacion.Producto no encontrado");
                    modelo.put("accion", "/editar");
                    modelo.put("titulo", "Editar producto");
                    modelo.put("boton", "Editar");
                    ctx.render("Publico/editarcrearprod.html", modelo);
